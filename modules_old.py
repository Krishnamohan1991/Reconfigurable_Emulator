...
def ConfNextSBPos(currentSBId,currentSBFace,nextSBId,fromSBPortIndex,route,route_len,count,freeport):
	...
	while((count+1)<(route_len-1)):
		...
		if(nxtSBpos_wrt_currSB==0):
			...
			print 'currentSBId %s A freeport %s '%(currentSBId,freeport)
			...
			print 'currentSBId %s currentSBFace %s freeport %s'%(currentSBId,currentSBFace,freeport)
			if(count==(route_len-2)):
				return {'currentport':fromSBPortIndex,'currentSBId':route[route_len-2],'currentSBFace':currentSBFace}		
			else:
				ConfNextSBPos(currentSBId,currentSBFace,route[count],fromSBPortIndex,route,route_len,count,freeport)
			...
			currentSBId=nextSBId
			count=count+1	
			print 'currentSBId %s currentSBFace %s freeport %s count %s'%(currentSBId,currentSBFace,freeport,count)
			if(count==(route_len-2)):
				return {'currentport':fromSBPortIndex,'currentSBId':route[route_len-2],'currentSBFace':currentSBFace}		
			else:
				ConfNextSBPos(currentSBId,currentSBFace,route[count],fromSBPortIndex,route,route_len,count,freeport)
	...
	#currentport=fromSBPortIndex
	#return {'currentport':fromSBPortIndex,'currentSBId':currentSBId,'currentSBFace':currentSBFace}			
			

def routeSignal(startSBId,startSBFace,startSBPortIndex,route,route_len,targetLUT):
	
	if(route_len>3):
		if(SB_connect.has_key(route[1]) and SB_connect.has_key(route[2])):
			nxtSBpos_wrt_currSB=FindNextSBPos(route[1],route[2])

			if(nxtSBpos_wrt_currSB==0):
				toFace='A'
				toFaceIndex=checkFreeSBPort(startSBId,'A')
		
			elif(nxtSBpos_wrt_currSB==1):
				toFace='B'
				toFaceIndex=checkFreeSBPort(startSBId,'B')
		
			elif(nxtSBpos_wrt_currSB==2):
				toFace='C'
				toFaceIndex=checkFreeSBPort(startSBId,'C')
		
			else:
				toFace='D'
				toFaceIndex=checkFreeSBPort(startSBId,'D')
			
			for i in range(1,route_len-1):
				route_list=routing(startSBId,startSBFace,startSBPortIndex,toFace,toFaceIndex,route,i)
				if route_list!=None:
					print 'route list %s'%route_list
					startSBId=route[i+1]
					startSBFace=route_list['start_Face']
					startSBPortIndex=route_list['start_index']
					toFace=route_list['target_Face']
					toFaceIndex=route_list['target_index']
			print 'CB %s'%route[route_len-1]
			finalCBIndex=conf_lastSB(startSBId,startSBFace,startSBPortIndex,LUT_connect[targetLUT][1],route,targetLUT)
	else:
		finalCBIndex=conf_lastSB(startSBId,startSBFace,startSBPortIndex,LUT_connect[targetLUT][1],route,targetLUT)
	return finalCBIndex
...
def routing(startSBId,startSBFace,startSBPortIndex,toFace,toFaceIndex,route,count):
	if(count < len(route)-1 and SB_connect.has_key(route[count+1]) and SB_connect.has_key(route[count+2])):
		switchBlock.configSwitchBlock(SBobjectDictionary[startSBId],startSBId,startSBFace,str(startSBPortIndex),toFace,str(toFaceIndex))
		nxtSBpos_wrt_currSB=FindNextSBPos(route[count+1],route[count+2])
		currentSBId=route[count+1]
		if(nxtSBpos_wrt_currSB==0):
			t_Face='A'
			t_FaceIndex=checkFreeSBPort(currentSBId,'A')
			
		elif(nxtSBpos_wrt_currSB==1):
			t_Face='B'
			t_FaceIndex=checkFreeSBPort(currentSBId,'B')
			##return {'start_Face':toFace,'start_index':toFaceIndex,'target_Face':t_Face,'target_index':t_FaceIndex}
		
		elif(nxtSBpos_wrt_currSB==2):
			t_Face='C'
			t_FaceIndex=checkFreeSBPort(currentSBId,'C')
			#return {'start_Face':toFace,'start_index':toFaceIndex,'target_Face':t_Face,'target_index':t_FaceIndex}
		
		else:
			t_Face='D'
			t_FaceIndex=checkFreeSBPort(currentSBId,'D')
		return {'start_Face':toFace,'start_index':toFaceIndex,'target_Face':t_Face,'target_index':t_FaceIndex}
	else:
		return None	
	
def connectRoute(route,fromCB,toCB,originLUT,targetLUT):  #function connects fromCB to tragetCB through all switch blocks
	#s={}  #kp
	route_len= len(route) # actual route index id from 0 to len-1
	count=1
	if CB_connect[fromCB][1]==route[1]: #check if the next SB is to the left of the origin-CB
		...
		'''
	i=4
	if(lutobjectDictionary[originLUT].status==1):   #origin LUT should have status 1 , menaing it has already been used once 
		...
		if(1==2):
			...
			if(CB_connect[fromCB][5]==originLUT):  #check if origin LUT is q1
				...
				final_CB_port=routeSignal(SBId,SBFace,fromSBPortIndex,route,route_len,targetLUT)
			...
			if(CB_connect[fromCB][6]==originLUT):
				...
				final_CB_port=routeSignal(SBId,SBFace,fromSBPortIndex,route,route_len,targetLUT)
				#switch_adj_toCB=ConfNextSBPos(SBId,SBFace,route[count+1],fromSBPortIndex,route,route_len,count,0)
			#elif(CB_connect[fromCB][6]==originLUT and CB_connect[fromCB][5]==originLUT):  #kp - not neccessary
			#	print 'incorrect origin LUT or origin CB'
			
	else:
		print 'cannot do routing.Input LUT is not producing any signal'	
			
				
	#CBobjectDictionary[fromCB].configCB(fromCB,'X','X','X','X',q1,q2)
	#SBobjectDictionary[SBId].setFaceStatus(SBFace,fromSBPortIndex,'Q') #relocated to within 
	'''
	...
	#switch_adj_toCB=ConfNextSBPos(SBId,SBFace,route[count+1],fromSBPortIndex,route,route_len,count,0)
	#print switch_adj_toCB  kp
	#return switch_adj_toCB	kp
	return final_CB_port
...
def conf_lastSB(SBId,fromSBFace,fromSBPortIndex,toCB,route,targetLUT):  #kp the fuction that connects the last SB in the route to the target CB
	...
	for i in stats:
		...
		count=count+1
	'''
	print 'joidfjoifjdoifdjofij %s'%toCB
